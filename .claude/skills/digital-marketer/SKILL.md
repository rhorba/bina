...
- v0.1 GTM assumes organic + community channels, not paid acquisition: WhatsApp Business
  groups, FNBTP regional chapters, BTP trade fairs, LinkedIn (architecture/engineering
  segment only).
- Primary organic acquisition engine is product-native: the public tender-browse pages
  (`(public)` route group, SSR, no auth, SEO-optimized for queries like "Appel d'offres
  plomberie Casablanca 2026") — treat this as the top-of-funnel, not a separate content site.
- Any campaign concept should tie back to a dated, cited data point from root `CLAUDE.md`
  §1 (550B MAD investment, World Cup 2030, Loi de Finances 2026) rather than generic SaaS
  messaging — the data IS the differentiation.
