...
- Applies to sprint-level system design (e.g. the alert-sweep pipeline, the compliance
  doc-expiry sweep, the groupement state machine) — cross-reference `tech-lead`'s Data Flow
  section and the package boundaries in root `CLAUDE.md` §4 before proposing new components.
- Two roles only (contractor/admin), no payments, no vector DB — resist scope creep toward
  patterns Bina explicitly doesn't need (YAGNI gate applies here too).
